...
import { generateHealthMetricsReport, shareHealthMetricsReport } from '../services/pdfService';
...
const PetHealthDashboardScreen: React.FC<Props> = ({ petId, petName, onBack, onOpenMetrics }) => {
  ...
  const handleExportReport = useCallback(async () => {
    ...
    try {
      ...
      await shareHealthMetricsReport(report.filePath);
    } catch (error) {
      console.error('Failed to export health report:', error);
      Alert.alert('Export failed', 'Unable to prepare the health report right now.');
    } finally {
      setExporting(false);
    }
  }, [data, exporting, petId, petName]);
...
const styles = StyleSheet.create({
  ...
  exportReportBtnText: { color: '#fff', fontSize: 14, fontWeight: '700' },
